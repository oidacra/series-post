...
describe('SearchComponent', () => {
	...
	describe('[GOOD] Tests', () => {
		it('should exist a input for the query control', () => {
			const queryInput = de.nativeElement.querySelector('[data-testId="query-input"]');
			expect(queryInput).not.toBeNull();
		});

		it('should show loading button when input is loading `true`', () => {
			// arrange
			const button = fixture.debugElement.query(By.directive(NzButtonComponent));
		...
		it('should emit form value when the user clicks the button and form is valid', async () => {
			...
			// Buscamos el botón
			const button = de.nativeElement.querySelector('[data-testId="submit-button"]');
		...
		describe('Clear icon', () => {
			it('clear icon should not be present if query is empty', () => {
				// Assert
				const clearIcon = de.nativeElement.querySelector('[data-testId="reset-icon"]');
